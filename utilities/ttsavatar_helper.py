...
def createAvatar(input_text, speech_region, speech_host, speech_api_key): 
    job_id = submit_synthesis(input_text, speech_region, speech_host, speech_api_key)
    if job_id is not None:
        while True:
            status, url = get_synthesis(job_id, speech_region, speech_host, speech_api_key)
            if status == 'Succeeded':
...
def submit_synthesis(input_text, speech_region, speech_host, speech_api_key, avatar_voice="it-IT-IsabellaNeural"):
    ...
    payload = {
        ...
        "properties": {
            ...
            "backgroundColor": "#d3d3d3", # background color in RGBA format, default is white; can be set to 'transparent' for transparent background
        }
    }
...
def get_synthesis(job_id, speech_region, speech_host, speech_api_key):
...
def list_synthesis_jobs(speech_region, speech_host, speech_api_key, skip: int = 0, top: int = 100):
    """List all batch synthesis jobs in the subscription"""
    url = f'https://{speech_region}.{speech_host}/api/texttospeech/3.1-preview1/batchsynthesis/talkingavatar?skip={skip}&top={top}'
    header = {
        'Ocp-Apim-Subscription-Key': speech_api_key
    }
    response = requests.get(url, headers=header)
